function [x,y,typ] = FLEX_gpin(job,arg1,arg2)
  x=[];y=[];typ=[];
  select job
  ...
  case 'set' then
    ...
    while %t do
      [ok,gpin_pin,exprs]=..
      getvalue('Set parameters for block GPIN',..
      ['GPIN pin [1..8] :'],..
      list('vec',-1),exprs)
      if ~ok then 
  ...
  case 'define' then
    ...
    gr_i=['xstringb(orig(1),orig(2),[''FLEX-GPIN'' ; ''Pin: ''+string(gpin_pin)],sz(1),sz(2),''fill'');']
