function [x,y,typ] = FLEX_MTB_pwm(job,arg1,arg2)
  x=[];y=[];typ=[];
  select job
  case 'plot' then
  ...
  case 'define' then
    pwm_pin=1
    model=scicos_model()
    model.sim=list('flex_daughter_pwm',4)
    ...
    gr_i=['xstringb(orig(1),orig(2),[''FLEX-MTB'' ; ''PWM Pin: ''+string(pwm_pin)],sz(1),sz(2),''fill'');']
