function [x,y,typ] = FLEX_canout(job,arg1,arg2)
  ...
  case 'set' then
    ...
    while %t do
      ...
      ['CAN Message ID:';..
             'Number of elements to send (1 or 2):'],..
      ...
      in = ones(n_channels,1);
      //model.in = ones(n_channels,1);
      out=[]
  ...
  case 'define' then
    ...
    exprs = [sci2exp(can_msg_id);..
                     sci2exp(n_channels)];
    gr_i =['xstringb(orig(1),orig(2),[''FLEX CAN OUT'' ; ''MSG ID: ''+string(can_msg_id); ''MSG DIM: ''+string(n_channels)],sz(1),sz(2),''fill'');'];
