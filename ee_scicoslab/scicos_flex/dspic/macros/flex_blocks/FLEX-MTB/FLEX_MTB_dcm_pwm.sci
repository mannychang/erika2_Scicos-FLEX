function [x,y,typ] = FLEX_MTB_dcm_pwm(job,arg1,arg2)
  x=[];y=[];typ=[];
  select job
  ...
  case 'set' then
    ...
    while %t do
      [ok,pwm_pin,exprs]=..
      getvalue('Select FLEX-MTB PWM Output',..
      ['PWM Out [1..2] :'],..
      list('vec',-1),exprs)
  ...
  case 'define' then
    pwm_pin=1
    model=scicos_model()
    model.sim=list('flex_daughter_dcm_pwm',4)
    ...
    gr_i=['xstringb(orig(1),orig(2),[''FLEX-MTB'' ; ''DCM Pin: ''+string(pwm_pin)],sz(1),sz(2),''fill'');']
