function [x,y,typ] = AMAZING_leds(job,arg1,arg2)
  x=[];y=[];typ=[];
  select job
  ...
  case 'set' then
    ...
    while %t do
      ...
      if(gpout_pin<1 | gpout_pin>2) then
        warning('Accepted values for led are in [1,2]. Keeping previous value.');
        break;
      end
  ...
  case 'define' then
    ...
    gr_i=['xstringb(orig(1),orig(2),[''AMAZING'' ; ''LED: ''+string(gpout_pin)],sz(1),sz(2),''fill'');']
